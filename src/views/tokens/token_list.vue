      ...
      >
        <tbody>
          ...
          <tr v-for="(item, index) in paginatedHistoryData" :key="index">
            <td class="text-theme">
              <text
                v-if="item.account.data.parsed.info.extensions"
                style="cursor: pointer"
                @click="pubbleys(item.pubkey)"
              >
                <span
                  v-for="(items, indexs) in item.account.data.parsed.info
                    .extensions"
                  :key="indexs"
                  class="colorfff"
                >
                  <img
                    v-if="
                      items.state && items.state.uri != 'http://www.baidu.com'
                    "
                    :src="items.state.uri"
                    width="32"
                    alt=""
              ...
              <text style="cursor: pointer" @click="pubbleys(item.pubkey)">
                {{ titleUrl(item.pubkey).url }}</text
              >

              <img
                v-if="titleUrl(item.pubkey).type"
                src="../../../src//assets//renzheng.png"
                height="24"
                alt=""
              />
            </td>
            ...
            <td class="text-theme">
              <text
                style="cursor: pointer"
                @click="pubbleys(item.account.owner)"
