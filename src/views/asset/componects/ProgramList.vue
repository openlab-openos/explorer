          ...
          <tbody>
            ...
            <tr v-for="item in paginatedHistoryData" :key="item.state.mint">
              ...
              <td>
                <!-- 显示加载状态或实际数量 -->
                {{
                  holdersCache[item.state.mint] === null
                    ? "loading..."
                    : holdersCache[item.state.mint]
                }}
                <!-- {{ item.holders }} -->
              </td>
              <td>
                ...
                {{
                  come(
                    smartFormatNumber(toFexedStake(item.supply, item.decimals))
                  )
                }}
...
const defaultImage =
  "https://r2.boss.top/images/20250606/bvxDCiRTIturmM0IwWtzWl89Ier8XJd27XqRpdHR.png"; // 默认图片
