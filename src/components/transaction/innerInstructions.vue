            ...
            <table class=" w-100 mb-0 small align-middle table table-striped table-borderless mb-2px small">
                ...
                <tbody>
                        ...
                        <tr v-for="[key, value] in Object.entries(item.parsed.info)" :key="key">
                            ...
                            <td class="text-end" :class="typeof titleUrl(value).url == 'string' ? 'text-theme' : ''"
                                ...
                                {{ key == 'extensionTypes' ? value[0] :(key == 'lamports' ? toFexedStake(value) : (key == 'tokenAmount' ? value.uiAmount : titleUrl(value).url) ) }} {{ key == 'space' ?
                                    'byts(s)' : '' }}
                                {{ key == "lamports" ? '(BTG)' : '' }}
                      <img v-if="titleUrl(value).type" v-for="(datas,indexs) in titleUrl(value).certificates" :key="indexs" :src="datas.img" width="16" class="marginRight8" alt="">
                    ...
                    <template v-if="item.accounts">
                      ...
                      <img v-if="titleUrl(item.accounts[0]).type" v-for="(datas,indexs) in titleUrl(item.accounts[0]).certificates" :key="indexs" :src="datas.img" width="16" class="marginRight8" alt="">
                      ...
                      <img v-if="titleUrl(item.data).type" v-for="(datas,indexs) in titleUrl(item.data).certificates" :key="indexs" :src="datas.img" width="16" class="marginRight8" alt="">
