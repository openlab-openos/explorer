              ...
              <tr v-for="(log, index) in paginatedHistoryData" :key="index">
                ...
                <td style="text-align: left; height: 40px" v-if="log.pubkey">
                  <span class="text-theme">
                    ...
                    <router-link
                      :to="{ name: 'address', params: { url: log.votepubkey } }"
                      >{{ stringcate(log.votepubkey) }}</router-link
                    >
                  </span>
                </td>
...
const getActivityLogData = async () => {
  ...
  if (res.length !== 0) {
    ...
    // });
    list.sort((a, b) =>
      new Intl.Collator("zh", { numeric: true }).compare(a.name, b.name)
    );
