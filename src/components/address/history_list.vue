        ...
        <tr v-for="(item, index) in paginatedHistoryData" :key="index">
          <td class="text-theme">
            ...
            <router-link
              ...
              >{{ item.signature }}</router-link
            >
          </td>
          <td :class="item.programId ? 'text-theme' : ''">
            <text>{{
              item.type == "unknown" ? "N/A" : textValue(item.type)
            }}</text>
          </td>
          <td :class="item.programId ? 'text-theme' : ''">
            <text v-if="item.programId" >
              <RenderText :address="item.programId" />
            </text>
...
const requestList = async (object) => {
  try {
    const response = await transactionRequest(object);
    console.log(response);

    // 解析和处理返回的数据
    return response.data; // 现在这个函数会返回解析后的数据
  } catch (error) {
