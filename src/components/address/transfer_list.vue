        ...
        <template v-if="loading && type">
          <tr v-for="(item, index) in paginatedHistoryData" :key="index">
            <td class="text-theme">
              ...
              <router-link
                :to="{ name: 'tx', params: { item: item.signature } }"
                >{{ stringcate(item.signature) }}</router-link
              >
            </td>
            <td>
              <button
            ...
            <td
              ...
              :class="item.primaryType ? 'color0-255-179-1' : 'text-theme'"
            >
            ...
            <td
              v-if="props.toType"
              style="text-align: left"
              :class="item.quantityType ? 'color0-255-179-1' : 'text-theme'"
            >
              <!-- <text style="cursor: pointer" @click="slot(
                                item.quantity
                            )"> {{
            ...
            <td style="text-align: left">
              ...
              <template v-if="URL_title">
                <text v-if="URL_title[item.token]">
                  <img
                    :src="URL_title[item.token].uri"
                    height="24"
                    class="marginRight8"
                    alt=""
                  />
                  {{
                    URL_title[item.token] ? URL_title[item.token].name : null
                  }}
                  {{
                    URL_title[item.token]
                      ? "(" + URL_title[item.token].symbol + ")"
                      : null
                  }}
                </text>
                <template v-else>
  ...
  for (let i in listItem) {
    try {
      ...
      const movementType =
        ...
        firstInstruction?.programId == "11111111111111111111111111111111"
          ? "Transfer"
          : "token_transfer";
      ...
      lamports = firstInstruction?.parsed?.info?.tokenAmount
        ? firstInstruction?.parsed?.info?.tokenAmount?.uiAmount
        : firstInstruction?.parsed?.info?.lamports / 1000000000;
      owner =
      ...
      // }
      let token = firstInstruction.parsed.info.mint
        ? titleUrl(firstInstruction.parsed.info.mint).url
        : "BTG";
