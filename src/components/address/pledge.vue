    ...
    <tbody>
      ...
      <template v-if="loading && type">
        <tr v-for="(item, index) in paginatedHistoryData" :key="index">
          <td class="text-theme">
            <text style="cursor: pointer" @click="pubbtx(item.pubkey)">{{
              item.pubkey
            }}</text>
          </td>
            ...
            {{
              percent(
                item.account.lamports
                  ? item.account.lamports / 1000000000
                  : "N/A"
              )
            }}
          </td>
          <td v-else>
            {{
              percent(
                item.account.data.parsed.info.stake
                  ? item.account.data.parsed.info.stake.delegation.stake /
                      1000000000
                  : "N/A"
              )
            }}
          ...
          <td
            :style="
              item.state == 'active'
                ? 'color:rgba(60,210,165,1)'
                : item.state == 'inactive'
                  ? 'color:rgba(60,210,165,1)'
                  : 'color:rgba(225,250,7,1)'
            "
          >
            {{ item.state }}
          </td>
...
onMounted(async () => {
  ...
  for (let i in historyData.value) {
    historyData.value[i].state = await stateFunction(
